...
void net_error(struct mbuf *m, int err) {
  ...
  if (unlikely(!iphdr)) return;
  if (unlikely(!ip_hdr_supported(iphdr))) return;
...
static void net_rx_one(struct mbuf *m) {
  ...
  iphdr = mbuf_pull_hdr_or_null(m, *iphdr);
  if (unlikely(!iphdr)) goto drop;

  /* Did HW checksum verification pass? */
  if (m->csum_type != CHECKSUM_TYPE_UNNECESSARY) {
    if (chksum_internet(iphdr, sizeof(*iphdr))) goto drop;
  }

  if (unlikely(!ip_hdr_supported(iphdr))) goto drop;
  len = ntoh16(iphdr->len) - sizeof(*iphdr);
  if (unlikely(mbuf_length(m) < len)) goto drop;
  if (len < mbuf_length(m)) mbuf_trim(m, mbuf_length(m) - len);
...
void net_rx_batch(struct mbuf **ms, unsigned int nr) {
  ...
  for (i = 0; i < nr; i++) {
    if (i + RX_PREFETCH_STRIDE < nr) prefetch(ms[i + RX_PREFETCH_STRIDE]->data);
    net_rx_one(ms[i]);
  }
}
...
static void iokernel_softirq_poll(struct kthread *k) {
  ...
  while (true) {
    if (!lrpc_recv(&k->rxq, &cmd, &payload)) break;

    switch (cmd) {
      case RX_NET_RECV:
        hdr = shmptr_to_ptr(&netcfg.rx_region, (shmptr_t)payload,
                            MBUF_DEFAULT_LEN);
...
static void __noinline net_tx_drain_overflow(void) {
  ...
  while (!mbufq_empty(&k->txpktq_overflow)) {
    ...
    if (net_ops.tx_single(m)) break;
    mbufq_pop_head(&k->txpktq_overflow);
    if (unlikely(preempt_cede_needed(k))) return;
  }
}
...
static void net_tx_raw(struct mbuf *m) {
  ...
  if (unlikely(!mbufq_empty(&k->txpktq_overflow))) net_tx_drain_overflow();
...
struct eth_addr all_dhosts[5] = {{{0x9c, 0xdc, 0x71, 0x5d, 0x41, 0x31}},
								{{0x9c, 0xdc, 0x71, 0x5d, 0x57, 0x11}},
								{{0x9c, 0xdc, 0x71, 0x5d, 0xd5, 0xb1}},
								{{0x9c, 0xdc, 0x71, 0x4b, 0x83, 0x61}},
								{{0x9c, 0xdc, 0x71, 0x5d, 0x67, 0x31}}};
...
int net_tx_ip(struct mbuf *m, uint8_t proto, uint32_t daddr) {
  ...
  /* route loopbacks */
  if (daddr == netcfg.addr) return net_tx_local_loopback(m, proto);
...
//   }
  ...
  for (i = 0; i < n; i++) net_tx_eth(ms[i], ETHTYPE_IP, dhost);
...
static void net_dump_config(void) {
  ...
  log_info("  mac:\t\t%02X:%02X:%02X:%02X:%02X:%02X", netcfg.mac.addr[0],
           netcfg.mac.addr[1], netcfg.mac.addr[2], netcfg.mac.addr[3],
           netcfg.mac.addr[4], netcfg.mac.addr[5]);
  log_info("  mtu:\t\t%d", net_get_mtu());
}

static int steer_flows_iokernel(unsigned int *new_fg_assignment) { return 0; }

static int register_flow_iokernel(unsigned int affininty, struct trans_entry *e,
                                  void **handle_out) {
  return 0;
}
...
int net_init(void) {
  ...
  ret = mempool_create(
      &net_tx_buf_mp, iok.tx_buf, iok.tx_len, PGSIZE_2MB,
      align_up(net_get_mtu() + MBUF_HEAD_LEN + MBUF_DEFAULT_HEADROOM,
               CACHE_LINE_SIZE * 2));
  if (ret) return ret;

  net_tx_buf_tcache = mempool_create_tcache(&net_tx_buf_mp, "runtime_tx_bufs",
                                            TCACHE_DEFAULT_MAG_SIZE);
  if (!net_tx_buf_tcache) return -ENOMEM;

  log_info("net: started network stack");
  net_dump_config();

#ifdef DIRECTPATH
  if (!cfg_directpath_enabled) net_ops = iokernel_ops;
#else
